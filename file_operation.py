...
def readServerReward(index, train):
    ...
    result = {'index': rewards[0], 'server_people_num': rewards[1], 'server_confidence_sum': rewards[2],
              "server_process_time": rewards[3],
              "server_transmission_time_selftest": rewards[4], "server_transmission_time_4g": rewards[5],
              "server_transmission_time_5g": rewards[6]}
    return result
...
def TestVideoOffloadEnv():
    ...
    for i in range(len1):
        ...
        print("index = " + str(i) + ", local_reward = " + str(local_reward) + ", server_reward = " + str(server_reward)
              + ", local_r1 = " + str(local_r1) + ", local_r2 = " + str(local_r2) + ", offload = " + str(
            local_reward < server_reward))

    print("local = " + str(local) + ", server = " + str(server) + ", max_reward = " + str(
        max_reward) + ", local_reward_sum = " + str(local_reward_sum) + ", server_reward_sum = " + str(
        server_reward_sum))
...
def TestTrainEnv():
    ...
    for i in range(len1):
        ...
        server_transmission_time_selftest = LayeringReward(
            readServerReward(i, train)['server_transmission_time_selftest'])
